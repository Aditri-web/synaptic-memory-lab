    ...
    <div className="w-full flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-2">
        ...
        <select
          ...
          className="bg-slate-900 text-slate-200 text-xs rounded border border-slate-700 px-2.5 py-1 font-mono focus:border-indigo-500 outline-none"
        >
          {results.map((r, idx) => (
      ...
      <div className="grid grid-cols-3 gap-2 bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80 text-center">
        <div>
          ...
          <span className="text-xs font-semibold text-indigo-300 font-mono truncate block">
            {current.keyLabel}
          </span>
        </div>
        <div>
          ...
          <span className={`text-xs font-bold font-mono ${
            current.cosineSimilarity >= 0.75 ? 'text-emerald-400' : current.cosineSimilarity >= 0.60 ? 'text-amber-400' : 'text-rose-400'
          }`}>
      ...
      <div className="flex flex-col gap-2 bg-slate-950 p-3 rounded-lg border border-slate-800">
        <div className="flex items-center justify-between text-[11px] text-slate-400">
          ...
          <span className="font-mono text-slate-500">Written Value</span>
        </div>
        <div className="grid grid-flow-col auto-cols-fr gap-1 h-6 items-end bg-slate-900/60 p-1 rounded border border-slate-800">
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-400">
          ...
          <span className="font-mono text-slate-500">Readout</span>
        </div>
        <div className="grid grid-flow-col auto-cols-fr gap-1 h-6 items-end bg-slate-900/60 p-1 rounded border border-slate-800">
          {Array.from(current.retrievedValue.slice(0, sampleDim)).map((val, i) => (
            <div
              ...
              style={{ height: `${Math.max(8, Math.min(100, val * 100))}%` }}
              className={`${
                current.isAccurate ? 'bg-indigo-500/80' : 'bg-rose-500/80'
