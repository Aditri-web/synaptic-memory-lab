...
export const SandboxSection: React.FC = () => {
  ...
  return (
    <section id="sandbox" className="w-full flex flex-col gap-6 py-6 border-b border-slate-200">
      {/* Section Header */}
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white hover:bg-slate-100 text-slate-700 text-xs font-mono border border-slate-300 shadow-xs transition-colors cursor-pointer"
        >
      ...
      <div className="flex items-center justify-between gap-3 text-[11px] font-mono text-slate-600 bg-slate-100/90 px-3.5 py-2 rounded-xl border border-slate-200">
        ...
        <span className="text-[10px] text-slate-600 bg-white px-2 py-0.5 rounded border border-slate-200 shadow-xs">
          60 FPS Client-Side Budget
        </span>
      </div>

      {/* Control Sliders Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 p-4 rounded-xl bg-white border border-slate-200 shadow-sm">
        {/* Slider 1: Stored Associations N */}
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between text-xs">
            <label htmlFor="input-stored-pairs" className="text-slate-700 font-semibold">Associations (N)</label>
            <span className="font-mono text-indigo-700 font-bold">{totalPairs}</span>
          </div>
          <input
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between text-xs">
            <label htmlFor="input-dimension-d" className="text-slate-700 font-semibold">Dimension (d)</label>
            <span className="font-mono text-indigo-700 font-bold">{dim} &times; {dim}</span>
          </div>
          <input
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between text-xs">
            <label htmlFor="input-learning-rate-eta" className="text-slate-700 font-semibold">Plasticity Rate (&eta;)</label>
            <span className="font-mono text-indigo-700 font-bold">{eta.toFixed(2)}</span>
          </div>
          <input
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between text-xs">
            <label htmlFor="input-decay-lambda" className="text-slate-700 font-semibold">Retention (&lambda;)</label>
            <span className="font-mono text-indigo-700 font-bold">{lambdaDecay.toFixed(2)}</span>
          </div>
          <input
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between text-xs">
            <label htmlFor="input-sparsity-ratio" className="text-slate-700 font-semibold">Active Sparsity</label>
            <span className="font-mono text-indigo-700 font-bold">{(sparsity * 100).toFixed(0)}%</span>
          </div>
          <input
      ...
      {capacityWarning ? (
        <div className="flex items-start gap-3 p-3.5 rounded-xl bg-amber-50 border border-amber-300 text-amber-900 text-xs shadow-xs">
          <AlertOctagon className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
          <div>
            <strong className="font-bold text-amber-950">Falsification Threshold Reached (Cross-talk Zone):</strong>
        ...
        <div className="flex items-center gap-3 p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs shadow-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-5 flex flex-col gap-3 p-4 rounded-xl bg-white border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase text-slate-800">
              Live Synaptic Matrix ($S_t$)
            </span>
            <span className="text-[10px] text-slate-600 bg-slate-100 px-2 py-0.5 rounded border border-slate-200 font-mono">
          ...
          <div className="flex flex-col gap-1 mt-2 bg-slate-50 p-3 rounded-lg border border-slate-200 shadow-xs">
            <div className="flex justify-between items-center text-[10px] uppercase font-mono text-slate-700 mb-1">
                  ...
                  >
                    <div
                      ...
                      className={`w-full rounded-t-sm transition-all ${
                        isGood ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-rose-500 hover:bg-rose-600'
                      }`}
                    />
                    <div className="hidden group-hover:block absolute bottom-full mb-1 bg-slate-900 text-[9px] px-1.5 py-0.5 rounded border border-slate-800 font-mono text-white whitespace-nowrap z-10 shadow-md">
